...
# based on https://github.com/Baymax-chen/EMVD/blob/main/inference.py`
SIGMA = {
    '1600': (3.513262, 11.917691, 4.551663171601465),
    '3200': (6.955588, 38.117816, 6.393192130541492),
    '6400': (13.486051, 130.818508, 8.288570575737536),
    '12800': (26.585953, 484.539790, 12.41668794312426),
    '25600': (52.032536, 1819.818657, 17.73972574092732)
}
...
def test_fastdvdnet(**args):
    ...
    iso = os.path.basename(args['test_path'])[3:]
    sigma_shot, sigma_bias, sigma_rgb = SIGMA[iso]
    ...
    with torch.no_grad():
        ...
        gt_seq, _, _ = open_sequence(args['gt_path'],
                                     args['gray'],
                                     expand_if_needed=False,
                                     max_num_fr=args['max_num_fr_per_seq'])
        seq = torch.from_numpy(seq).to(device)
        gt_seq = torch.from_numpy(gt_seq).to(device)
        seq_time = time.time()

        # Add noise
        noisestd = torch.FloatTensor([3.0*sigma_rgb/255.0]).to(device)
    ...
    stop_time = time.time()
    psnr = batch_psnr(denframes, gt_seq, 1.)
    psnr_noisy = batch_psnr(seq.squeeze(), gt_seq, 1.)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_path", type=str, default="./data/rgb/Kodak24",
                        help='path to gt sequence')
    ...
    assert os.path.basename(argspar.test_path)[:3] == 'ISO', argspar.test_path
